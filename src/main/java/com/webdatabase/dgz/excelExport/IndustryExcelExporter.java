...
public class IndustryExcelExporter {
	private XSSFWorkbook xssfWorkbook;
	private XSSFSheet xssfSheet;
	private List<Industry> listIndustries;
	
	public IndustryExcelExporter(List<Industry> listIndustries) {
		this.listIndustries = listIndustries;
		xssfWorkbook = new XSSFWorkbook();
	}
	
	private void writeHeaderLines() {
		xssfSheet = xssfWorkbook.createSheet("Industries");
	...
	private void writeDataLines() {
		...
		for(Industry industry : listIndustries) {
			Row row = xssfSheet.createRow(rowCount++);
			int columnCount = 0;
			
			createCell(row, columnCount++, industry.getId(), style);
			createCell(row, columnCount++, industry.getName(), style);
			createCell(row, columnCount++, industry.getCreatedAt(), style);
			createCell(row, columnCount++, industry.getUpdatedAt(), style);
		}
	}
